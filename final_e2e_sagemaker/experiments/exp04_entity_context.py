#!/usr/bin/env python3
"""EXP-04: add label-free entity-context features to the EXP-03 config-C pipeline.

Derived from exp03_e2e_rank.py with ONE change: featurize() appends 8
entity-context features computed from rr_score/rr_rank within each S1's own
candidate list. Retrieval, K=30, addr_max_df=400,000, the 62 pair features, the
LightGBM hyperparameters, the FIT/TUNE/EVAL split, hard-negative construction,
the threshold sweep and the EVAL sample are all untouched.

The 8 features use ONLY retrieval scores and ranks -- no labels, no ground
truth -- so they are computable at inference time for the test set.
...
DC = ROOT / "data_cache"
# the 8 added entity-context features, in matrix order
CTX_COLS = ["candidate_count", "normalized_rank", "best_rr_score",
            "second_best_rr_score", "score_to_best", "margin_to_best",
            "margin_to_second", "candidate_percentile"]
...
def featurize(cand: dict, s1n, pooln):
    ...
    # --- entity-context block: per-S1 statistics over its own candidate list ---
    # Label-free by construction: derived from rr / rank only.
    G = "source1_entity_id"
    second = (df.sort("rr", descending=True)
                .group_by(G, maintain_order=True)
                .agg(pl.col("rr").slice(1, 1).first().alias("second_best_rr_score")))
    df = (df.with_columns(pl.len().over(G).alias("candidate_count"),
                          pl.col("rr").max().over(G).alias("best_rr_score"))
            .join(second, on=G, how="left")
            .with_columns(pl.col("second_best_rr_score").fill_null(0.0))
            .with_columns(
                (pl.col("rank") / pl.max_horizontal(pl.col("candidate_count") - 1,
                                                    pl.lit(1))).alias("normalized_rank"),
                (pl.col("rr") / pl.col("best_rr_score").clip(lower_bound=1e-9)).alias("score_to_best"),
                (pl.col("best_rr_score") - pl.col("rr")).alias("margin_to_best"),
                (pl.col("rr") - pl.col("second_best_rr_score")).alias("margin_to_second"),
                (1.0 - pl.col("rank") / pl.col("candidate_count")).alias("candidate_percentile"),
            ))
    ...
    extra_cols = ["rr", "rank"] + CTX_COLS
    extra = np.column_stack([j[c].to_numpy() for c in extra_cols]).astype(np.float32)
    return (np.hstack([X, extra]),
            names + ["rr_score", "rr_rank"] + CTX_COLS,
            j.select("source1_entity_id", "cand_id"))
...
def run_cfg(tag, max_df_abs, k, idx_cache, S1, dfc_n, dfc_a, mx_n, cfg_base,
    ...
    L(f"  [{tag}] trained on {len(yf):,} pairs ({int(yf.sum()):,} pos, {len(names)} features)")
...
def main() -> int:
    ...
    res = []
    run_cfg("D_k30_df400000_ctx", 400000.0, 30, cache, S1, dfc_n, dfc_a, mx_n, cfg_base,
            fit, tune, ev, truth, s1n, pooln, bid, res)

    out = ROOT / "final_e2e_sagemaker" / "experiments" / "exp04_entity_context.json"
